...
class Basic(torch.nn.Module):
	def __init__(self, intLevel, arguments_strModel):
		...
		for intConv in range(5):
			self.moduleBasic[intConv * 2].weight.data.copy_(torch.load('FlowNetworks/flow_models/models/spynet_models/modelL' + str(intLevel + 1) + '_' + arguments_strModel  + '-' + str(intConv + 1) + '-weight.pth.tar'))
			self.moduleBasic[intConv * 2].bias.data.copy_(torch.load('FlowNetworks/flow_models/models/spynet_models/modelL' + str(intLevel + 1) + '_' + arguments_strModel  + '-' + str(intConv + 1) + '-bias.pth.tar'))
